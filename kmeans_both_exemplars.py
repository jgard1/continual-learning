...
def get_point_nearest_centroid(centroid, cluster_features, original_idxs):
...
def get_all_points_in_cluster(features, cluster_ids_x, cluster_number):
    ...
    original_dataset_idxs = ((cluster_ids_x == cluster_number).nonzero(as_tuple=True))
    
    print(original_dataset_idxs)
...
def get_cluster_exemplars(features, num_clusters):
    ...
    for cluster_number, centroid in enumerate(cluster_centers):
        ...
        cluster_features, cluster_to_original_idxs = get_all_points_in_cluster(features, cluster_ids_x, cluster_number)
        selected_feature, selected_feature_idx = get_point_nearest_centroid(centroid, cluster_features, cluster_to_original_idxs)
    ...
    return ret_features, original_idx_map
...
class ExemplarHandler(nn.Module, metaclass=abc.ABCMeta):
    ...
    def construct_exemplar_set(self, dataset, n):
        ...
        if self.herding:
            ...
            features_kmeans, original_idxs_map = get_cluster_exemplars(features, min(n, n_max))
            ...
            for k in range(min(n, n_max)):
                ...
                index_selected = original_idxs_map[shortlist_idx_selected]
                # END JOSH Memes mod ################################################################
                #####################################################################################
